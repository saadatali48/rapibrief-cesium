...
const Car3toDegrees = function (car3, viewer) {
  const cartographic =
    viewer.scene.globe.ellipsoid.cartesianToCartographic(car3)
...
const addPolygon = function (viewer, option) {
  const defaultPolygon = new Cesium.PolygonGraphics({
    ...
    outlineColor: option.outlineColor
      ? option.outlineColor
      : Cesium.Color.BLACK,
    outlineWidth: option.outlineWidth ? option.outlineWidth : 10,
...
function openFixedRotation(drawer) {
  ...
  drawer.viewer.scene.postUpdate.removeEventListener(
    inertial.bind(drawer, drawer)
  )
  drawer.viewer.scene.postUpdate.addEventListener(
    fixedRotation.bind(drawer, drawer)
  )
...
class Global {
  ...
  initialize(drawer, option) {
    ...
    drawer.viewer.cesiumWidget.screenSpaceEventHandler.removeInputAction(
  ...
  registerLeftClick(drawer, handler) {
    handler.setInputAction(
      this.viewLeftClick.bind(this, drawer),
  ...
  registerMouseMove(drawer, handler) {
    handler.setInputAction(
      this.viewMouseMove.bind(this, drawer),
  ...
  async viewLeftClick(drawer, ...event) {
    ...
    if (this.globalStatus === 'add') return
    ...
    // 如果是drawer绘制的实体
    if (entityType) {
      const eventRes = await D.emit(
        CesiumDrawer.EventTypeEnum.startEdit,
        entity.id,
        entityType
      )
      ...
      for (const drawerRepositoryElement of this.drawerRepository) {
        drawerRepositoryElement.emit(
          CesiumDrawer.EventTypeEnum.click,
  ...
  viewMouseMove(drawer, ...event) {
    ...
    this.timer = setTimeout(() => {
      ...
      if (entity) {
        // 暂时注释掉，鼠标移动到实体上就变
        if (document.body.style.cursor !== 'pointer') setCursorStyle('pointer')
...
class Graph {
  ...
  async cancelDraw() {
    if (!('id' in this.drawer.activeEntity)) return
    const res = this.drawer.emit(
      CesiumDrawer.EventTypeEnum.cancelDraw,
      this.drawer.activeEntity,
      this.drawer.activeEntityType
    )
  ...
  finishEdit() {
    ...
    const ID = this.drawer.activeEntity.id
    const res = this.drawer.emit(
      CesiumDrawer.EventTypeEnum.finishEdit,
      this.drawer.activeEntity,
      this.drawer.activeEntityType,
  ...
  insertNode(entity, option, index) {
    ...
    if (!('position' in option)) return undefined
    // 如果index不存在,则在末尾插入
  ...
  setCallback(entity, Car3Arr) {
    if (entity.polygon instanceof Cesium.PolygonGraphics) {
      entity.polygon.hierarchy = new Cesium.CallbackProperty(
        () => new Cesium.PolygonHierarchy(Car3Arr),
        false
      )
    }
  }
  ...
  offSetCallback(entity, Car3Arr) {
    if (entity.polygon instanceof Cesium.PolygonGraphics) {
      entity.polygon.hierarchy = new Cesium.PositionPropertyArray(
        Car3Arr.map((item) => new Cesium.ConstantPositionProperty(item))
      )
    }
  }
  /**
  ...
  isBelongActiveEntity(entity) {
    ...
    if (!('id' in this.drawer.activeEntity)) return false
    if (entity.id === this.drawer.activeEntity.id) return true
    const ID = this.drawer.activeEntity.id
    const res = this.pointEntities[ID].find((item) => item.id === entity.id)
    if (res) return true
    const mid = this.middlePointEntities[ID].find(
  ...
  setMiddlePoint(entity) {
    ...
    for (const [key, value] of arr.entries()) {
      ...
      if ('extensionPoint' in option) {
        ...
        if (!this.middlePointEntities[ID]) this.middlePointEntities[ID] = []
        if (!this.middlePointPositions[ID]) this.middlePointPositions[ID] = []
  ...
  middleToPeak(index) {
    if ('id' in this.drawer.activeEntity) {
      ...
      this.pointPositions[ID].splice(
        index,
        1,
        this.pointPositions[ID][index],
        position
      )
  ...
  addLeftClick(event) {
    ...
    this.drawer.emit(
      CesiumDrawer.EventTypeEnum.drawing,
      this.drawer.activeEntity,
      this.drawer.activeEntityType,
      this.pointPositions[ID]
    )
  ...
  addMouseMove(event) {
    ...
    setCursorStyle('default')
    // 如果不在地球上就返回
    if (!endEarthPosition) return
    // 如果位置在地球上存在
    setCursorStyle('crosshair')
    let ID
    if ('id' in this.drawer.activeEntity) ID = this.drawer.activeEntity.id
    if (
      'id' in this.activePointEntity &&
      'position' in this.activePointEntity
    ) {
      // 改变当前点的坐标
      this.activePointEntity.position = new Cesium.ConstantPositionProperty(
        endEarthPosition
      )
  ...
  async editLeftClick(event) {
    ...
    if (entityType) {
      this.finishEdit()
      this.drawer.init()
      const eventRes = await this.drawer.emit(
        CesiumDrawer.EventTypeEnum.startEdit,
        entity,
        entityType
      )
  ...
  editMouseMove(event) {
    ...
    if (this.isDragging) {
      ...
      if (!('id' in this.drawer.activeEntity)) return
      const ID = this.drawer.activeEntity.id
      const index = this.pointEntities[ID].findIndex(
        (item) => item.id === this.activePointEntity.id
      )
      this.activePointEntity.position = new Cesium.ConstantPositionProperty(
        endEarthPosition
      )
      ...
      setPosition(this.drawer.activeEntity, this.pointPositions[ID])
      this.drawer.emit(
        CesiumDrawer.EventTypeEnum.editing,
        this.drawer.activeEntity,
        this.drawer.activeEntityType,
        this.pointPositions[ID]
      )
      ...
      if (entity) {
        ...
        if (document.body.style.cursor !== 'default') setCursorStyle('default')
      }
    }
  }
  // 编辑状态，leftDown回调
  editLeftDown(event) {
    ...
    // 判断是否是当前活跃实体的中间点
    const index = this.middlePointEntities[ID].findIndex(
      (item) => item.id === entity.id.id
    )
    if (index > -1) {
  ...
  editRightClick(event) {
    ...
    if (entity.id.id === this.drawer.activeEntity.id) {
      setPosition(this.drawer.activeEntity, this.pointPositions[ID])
      this.drawer.emit(
        CesiumDrawer.EventTypeEnum.delete,
        this.drawer.activeEntity,
        this.drawer.activeEntityType
      )
      this.deleteEntity(this.drawer.activeEntity)
...
class Point extends Graph {
  ...
  startDraw(option) {
    ...
    this.drawer.emit(
      CesiumDrawer.EventTypeEnum.startDraw,
      this.drawer.activeEntity,
      'point'
    )
    // 切换状态
    this.drawer.init(
      'add',
      this.drawer.activeEntity,
      CesiumDrawer.EntityTypeEnum.point
    )
    // 保存配置信息
    this.option = option
  }
  ...
  async cancelDraw() {
    ...
    if (Object.is(res, false)) return
    if (!isCesiumEntity(this.drawer.activeEntity)) return
  ...
  addLeftClick(event) {
    ...
    // 发送点绘制完成的事件
    this.drawer.emit(
      CesiumDrawer.EventTypeEnum.finishDraw,
      this.drawer.activeEntity,
      earthPosition
    )
    // 绘制完成之后把实体存放一下
  ...
  addMouseMove(event) {
    ...
    if ('id' in this.drawer.activeEntity && this.drawer.activeEntity.id) {
      this.drawer.activeEntity.position = new Cesium.ConstantPositionProperty(
        endEarthPosition
      )
    } else {
  ...
  editRightClick(event) {
    ...
    if (entity && entity.id.id === this.drawer.activeEntity.id) {
      ...
      const index = this.entities.findIndex(
        (item) => item.id === this.drawer.activeEntity.id
      )
      this.viewer.entities.remove(this.drawer.activeEntity)
      this.entities.splice(index, 1)
      this.drawer.init()
...
class Polyline extends Graph {
  ...
  startDraw(option) {
    ...
    if (this.option.extensionPoint instanceof Cesium.PointGraphics) {
      ...
      this.option.extensionPoint = new Cesium.PointGraphics(
   ...
   */
  setCallback(entity, Car3Arr) {
    if (!(entity.polyline instanceof Cesium.PolylineGraphics)) return
  ...
  offSetCallback(entity, Car3Arr) {
    ...
    entity.polyline.positions = new Cesium.PositionPropertyArray(
      Car3Arr.map((car3) => new Cesium.ConstantPositionProperty(car3))
    )
  }
  /**
  ...
  setPeakPoint(entity) {
    const ID = entity.id
    if (
      entity.polyline instanceof Cesium.PolylineGraphics &&
  ...
  setMiddlePoint(entity) {
    ...
    for (const [key, value] of pointPositions.entries()) {
      ...
      if (key === pointPositions.length - 1) {
        ...
        position = computeMiddlePoint(
          pointPositions[key],
          pointPositions[key + 1]
        )
...
class Polygon extends Graph {
  ...
  startDraw(option) {
    this.option = option
    if (!this.option.polygon) this.option.polygon = new Cesium.PolygonGraphics()
    this.option.polygon.hierarchy = new Cesium.CallbackProperty(
   ...
   */
  setCallback(entity, Car3Arr) {
    if (!(entity.polygon instanceof Cesium.PolygonGraphics)) return
    entity.polygon.hierarchy = new Cesium.CallbackProperty(
      () => new Cesium.PolygonHierarchy(Car3Arr),
      false
    )
...
const addCzmlDocument = async function (viewer, option) {
  const document = {
    ...
    clock: {
      ...
      currentTime:
        option.currentTime || Cesium.JulianDate.fromDate(new Date()).toString(),
...
function generateSatelliteOptions(options) {
  ...
  for (let I = 0; I < Math.ceil(countRound); I++) {
    ...
    const start = Cesium.JulianDate.fromDate(
      new Date(startTimeStamp + I * roundTime * 1000)
    ).toString()
    const E =
      I === Math.ceil(countRound) - 1
        ? endTimeStamp
  ...
  if (options.position.degrees && options.position.degrees.length > 0) {
    ...
    for (let I = 0; I < options.position.degrees.length; I++) {
      position.cartographicDegrees.push(
        I * options.position.step,
        options.position.degrees[I].longitude,
        options.position.degrees[I].latitude,
        options.position.degrees[I].height
      )
    }
  }
  // 如果有笛卡尔3d坐标系集合就用笛卡尔
  if (options.position.cartesian && options.position.cartesian.length > 0) {
    ...
    for (let I = 0; I < options.position.cartesian.length; I++) {
      position.cartesian.push(
        I * options.position.step,
        options.position.cartesian[I].x,
        options.position.cartesian[I].y,
        options.position.cartesian[I].z
      )
    }
  }
  return {
  ...
  while (stack.length > 0) {
    ...
    if (Array.isArray(node.data)) {
      ...
      for (const dataKey in node.data) {
        if (!Object.prototype.hasOwnProperty.call(node.data, dataKey)) continue
        const index = dataKey
...
function merge(target, origin) {
  ...
  // 如果参数类型不一致直接返回target
  if (targetType !== originType) return target
  if (
    ...
    if (Array.isArray(node.data) && Array.isArray(node.target)) {
      if (node.key !== undefined && node.target[node.key] === undefined) {
        ...
        Object.setPrototypeOf(
          node.target[node.key],
          Object.getPrototypeOf(node.data)
        )
      }
      if (node.key === undefined) {
        Object.setPrototypeOf(node.target, Object.getPrototypeOf(node.data))
      ...
      for (const [index, value] of node.data.entries()) {
        // 如果已经复制过了则跳过
        if (cache.get(value) === index) continue
        stack.push({
          data: value,
          key: index,
...
class CesiumDrawer {
  ...
  init(
    status = 'view',
    entity = {},
    entityType = CesiumDrawer.EntityTypeEnum.initial
  ) {
  ...
  startDraw(option) {
    if (
      !['polyline', 'Circle', 'rectangle', 'polygon', 'point'].includes(
        option.type
      )
    ) {
  ...
  startEdit(entity, entityType) {
    ...
    if (!entityType)
      throw new ReferenceError(
        'This entity was not created by CesiumDrawer and does not support editing'
      )
    if (!drawer) return
    // 边界处理，如果当前正在绘制某个实体，则取消绘制
    const { activeDrawer } = this.global
  ...
  registerLeftDown(status, entityType) {
    ...
    this.handler.setInputAction(
      (this.graph.methodsMap[status].leftDown || fun).bind(this.graph),
  ...
  registerLeftUp(status, entityType) {
    ...
    // if (entityType !== "initial" && entityType !== "point") return;
    this.handler.setInputAction(
      (this.graph.methodsMap[status].leftUp || fun).bind(this.graph),
  ...
  registerLeftClick(status, entityType) {
    ...
    this.handler.setInputAction(
      // 需要用bind指定当前实例
      (this.graph.methodsMap[status].leftClick || fun).bind(this.graph),
      Cesium.ScreenSpaceEventType.LEFT_CLICK
    )
  }
  /**
  ...
  registerMouseMove(status, entityType) {
    ...
    // if (entityType !== "initial" && entityType !== "point") return;
    this.handler.setInputAction(
      (this.graph.methodsMap[status].mouseMove || fun).bind(this.graph),
      Cesium.ScreenSpaceEventType.MOUSE_MOVE
    )
  }
  /**
  ...
  registerRightClick(status, entityType) {
    ...
    // if (entityType !== "initial" && entityType !== "point") return;
    this.handler.setInputAction(
      (this.graph.methodsMap[status].rightClick || fun).bind(this.graph),
  ...
  registerLeftDoubleClick(status, entityType) {
    ...
    // if (entityType !== "initial" && entityType !== "point") return;
    this.handler.setInputAction(
      (this.graph.methodsMap[status].leftDoubleClick || fun).bind(this.graph),
      Cesium.ScreenSpaceEventType.LEFT_DOUBLE_CLICK
    )
  }
  // 注销鼠标左键按下
  ...
  finishEditing() {
    this.global.activeDrawer.classMap[
      this.global.activeDrawer.activeEntityType
    ].finishEdit()
  }
}

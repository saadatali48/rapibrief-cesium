...
class Graph {
  ...
  addMouseMove(event) {
    ...
    if (
      ...
      this.pointPositions[ID][
        this.pointPositions[ID].length - 1
      ] = endEarthPosition;
    } else {
  ...
  addRightClick() {
    ...
    this.pointPositions[ID].pop();
    this.pointEntities[ID].pop();
    this.activePointEntity = this.pointEntities[ID][
      this.pointEntities[ID].length - 1
    ];
    setPosition(this.drawer.activeEntity, this.pointPositions[ID]);
    this.drawer.emit(
      CesiumDrawer.EventTypeEnum.drawing,
...
class Point extends Graph {
  ...
  async editLeftClick(event) {
    ...
    if (!isCesiumEntity(this.drawer.activeEntity)) return;
    if (entity.id !== this.drawer.activeEntity.id) {
      const { entityType, drawer } =
        this.drawer.global.findDrawer(entity) || {};
      if (entityType) {
        this.finishEdit();
        this.drawer.init();
        const eventRes = await this.drawer.emit(
